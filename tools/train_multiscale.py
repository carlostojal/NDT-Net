...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_desired_nds", type=int, help="Number of desired normal distributions", default=8160, required=False)
    parser.add_argument("--n_desired_nds1", type=int, help="Second number of desired normal distributions", default=4080, required=False)
    ...
    # check if the n_desired_nds1 is set
    if args.n_desired_nds1 is not None:
        desired_nds.append(int(args.n_desired_nds1))
    ...
    for epoch in range(int(args.epochs)):
        ...
        acc = 0.0
        total_acc = 0.0
        for i, (pcl1, covs1, gt1, sampler1, pcl2, covs2, gt2, sampler2) in enumerate(train_loader):
            # move the data to the device
            pcl1 = pcl1.to(device) # [B, R, N, 3], where R is the number of resolutions
            covs1 = covs1.to(device)
            gt1 = gt1.to(device)
            pcl2 = pcl2.to(device)
            covs2 = covs2.to(device)
            gt2 = gt2.to(device)
            ...
            # forward pass
            pred = None
            # if single-scale model
            if "multiscale" not in args.task:
                ...
                pred = model(pcl1, covs1, sampler1[0], pcl2, covs2, sampler2[0]) # [B, N, C]

            # compute the loss - cross entropy
            loss = torch.nn.functional.cross_entropy(pred, gt1)
